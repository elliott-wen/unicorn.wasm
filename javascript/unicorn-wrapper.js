...
function createUC(MUnicornInstance) {
    ...
    const uc = {
        // Static
        version: function () {
            const major_ptr = MUnicornInstance._malloc(4);
            const minor_ptr = MUnicornInstance._malloc(4);
            const ret = MUnicornInstance.ccall('uc_version', 'number', ['pointer', 'pointer'], [major_ptr, minor_ptr]);
            const major = MUnicornInstance.getValue(major_ptr, 'i32');
            const minor = MUnicornInstance.getValue(minor_ptr, 'i32');
            MUnicornInstance._free(major_ptr);
            MUnicornInstance._free(minor_ptr);
            return { ret, major, minor };
        },

        arch_supported: function (arch) {
            return MUnicornInstance.ccall('uc_arch_supported', 'number', ['number'], [arch]);
        },

        strerror: function (code) {
            return MUnicornInstance.ccall('uc_strerror', 'string', ['number'], [code]);
        },

        /**
         * Unicorn instance object
         */
        Unicorn: function (arch, mode) {
            ...
            this.mem_regions = function () {
                console.error('Unicorn.js: mem_regions not implemented');
            };
            ...
            this.hook_add = function (type, user_callback, user_data, begin, end, extra) {
                const handle = MUnicornInstance.getValue(this.handle_ptr, '*');
                user_data = user_data || {};
                begin = (begin === undefined && end === undefined) ? 1 : begin;
                end = end || 0;
                let callback_ptr;
                ...
                if (ret !== uc.ERR_OK) {
                    ...
                    throw `uc_hook_add failed code ${ret}: ${uc.strerror(ret)}`;
                }
                const hook = { handle: MUnicornInstance.getValue(hook_ptr, '*'), callback: callback_ptr };
                MUnicornInstance._free(hook_ptr);
                return hook;
            };
            ...
            this.hook_del = function (hook) {
                const handle = MUnicornInstance.getValue(this.handle_ptr, '*');
                const ret = MUnicornInstance.ccall('uc_hook_del', 'number', ['pointer', 'pointer'], [handle, hook.handle]);
                if (ret !== uc.ERR_OK) throw `uc_hook_del failed code ${ret}: ${uc.strerror(ret)}`;
                MUnicornInstance.removeFunction(hook.callback);
            };
            ...
            this.emu_stop = function () {
                const handle = MUnicornInstance.getValue(this.handle_ptr, '*');
                const ret = MUnicornInstance.ccall('uc_emu_stop', 'number', ['pointer'], [handle]);
                if (ret !== uc.ERR_OK) throw `uc_emu_stop failed code ${ret}: ${uc.strerror(ret)}`;
            };

            this.errno = function () {
                const handle = MUnicornInstance.getValue(this.handle_ptr, '*');
                return MUnicornInstance.ccall('uc_errno', 'number', ['pointer'], [handle]);
            };

            this.close = function () {
                const handle = MUnicornInstance.getValue(this.handle_ptr, '*');
                const ret = MUnicornInstance.ccall('uc_close', 'number', ['pointer'], [handle]);
                if (ret !== uc.ERR_OK) throw `uc_close failed code ${ret}: ${uc.strerror(ret)}`;
            };

            // ======= Constructor =======
            const ret = MUnicornInstance.ccall('uc_open', 'number', ['number', 'number', 'pointer'], [this.arch, this.mode, this.handle_ptr]);
            if (ret !== uc.ERR_OK) {
                MUnicornInstance.setValue(this.handle_ptr, 0, '*');
                throw `uc_open failed code ${ret}: ${uc.strerror(ret)}`;
            }
        }
    };
